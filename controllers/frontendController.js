  ...
  pool.connect((err, client, done) => {
    if (err) next(err);
      ...
      if (post) {
        ...
        res.status(404);
        res.json({ message: 'Unable to find post.', err: 'Post not found.' })
      }
    }).catch(err => {
      res.status(503);
      console.log(err);
...
module.exports.createCommentPOST = [
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      const comment_id = v5(req.params.postId + req.body.date + req.body.name + req.body.body, process.env.SECRETUUID);
      const tryToFindDuplicateCommentQuery = `
      SELECT * FROM comments
      WHERE comment_id=($1);`;
      const response = await pool.query(tryToFindDuplicateCommentQuery, [comment_id])

      if (response.rowCount > 0) {
        res.locals.errors.push({ msg: 'Duplicate post detected. You must change the name or comment body slightly.' });
      } else {
  ...
  (req, res, next) => {
    ...
    if (res.locals.errors.length > 0) {
      ...
      pool.query(createCommentQuery, createCommentQueryValues)
        .then(value => {
          console.log(value);
          res.json({
